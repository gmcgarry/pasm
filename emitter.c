...
#endif
	if (a < addr) {
		warning("address changed to %lX, from %lX", a, addr);
		warning("binary output format can't write backwards");
		addr = a;
		return;
	}
}
...
void srec_finishline(void)
{
	...
	if (fout == NULL) return;
	srec_check = srec_index + (srec_address & 0xff) + ((srec_address>>8) & 0xff) + 3;
...
{
	if (ftype == NULL) {
		fatal("No S Record Format\n");
		return -1;
	}

	if (sscanf(ftype, "srec%d", &srec_format) != 1) {
		fatal("Illegal S Record format \"%s\", must be \"srec2\", \"srec3\", or \"srec4\"\n", ftype);
		return -1;
	}
